...
import type { CartTotals, ResolvedLine } from '@/lib/cart-store'
import { site } from '@/lib/site'
import { formatPrice } from '@/lib/utils'

/** One basket line, shown the same way in the sidebar and in the review step. */
function SummaryLine({ line }: { readonly line: ResolvedLine }) {
  return (
    <li className="flex gap-4 border-b border-dashed border-ink/15 pb-4 last:border-0 last:pb-0">
...
function SummaryTotals({ totals }: { readonly totals: CartTotals }) {
  return (
    <>
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="flex items-start gap-3 rounded-lg border-2 border-ink bg-lime-soft px-5 py-4">
        ...
        <p className="text-[0.86rem] leading-relaxed font-medium text-graphite">
          ...
          from {site.address.locality}, {site.address.regionCode} within 1 to 2
...
/**
 * The same basket, folded into the final review step.
 *
 * The sidebar copy is dropped here: by this point the customer has already
 * read it twice, and the step they are on repeats the "no charge" promise
 * right below this block anyway.
 */
export function OrderRecap() {
  ...
  if (!hydrated) {
    return (
      <div className="h-56 rounded-lg border-2 border-line bg-haze motion-safe:animate-pulse" />
    )
  }

  return (
    <div className="rounded-lg border-2 border-ink bg-cream p-5 sm:p-6">
      <div className="flex items-center justify-between gap-4">
        <h3 className="font-display text-xl font-extrabold">What you are ordering</h3>
        <Link
          href="/cart"
          className="inline-flex items-center gap-1.5 rounded-full border-2 border-ink bg-paper px-4 py-2 font-mono text-[0.62rem] font-bold tracking-[0.14em] text-ink uppercase transition-colors hover:bg-volt-soft hover:text-volt-deep"
        >
          <Pencil className="h-3 w-3" strokeWidth={3} aria-hidden="true" />
          Edit cart
        </Link>
